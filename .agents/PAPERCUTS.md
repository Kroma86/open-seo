...
- [ ] `2026-09-10T17:10:00Z` — `kimi` — `npx vitest run some.test.ts` silently runs zero tests (exit 2, "filter … include: src/**/*.test.ts") for any test file outside `src/` — e.g. a scratch check at the repo root. Either widen the vitest include or document that all test files must live under `src/`.
